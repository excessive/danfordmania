...
-- Preview
local preview = Def.Sprite {
	Texture     = string.format("preview %s", st),
...
-- Card Frame
local card_frame = Def.ActorFrame {
	InitCommand = function(self)
	...
	Def.Sprite {
		Texture = string.format("card frame %s", st),
	},
	Def.BitmapText {
	...
	Def.Sprite {
		Texture = string.format("card %s", pad_file),
		InitCommand = function(self)
			self:xy(-82, 8)
		end
	}
...
-- Icon Frame
local icon_frame = Def.ActorFrame {
	InitCommand = function(self)
		self:xy(SCREEN_CENTER_X + 20 + gc:GetIndex() * 106, SCREEN_CENTER_Y - 90)
	end,
	OnCommand = function(self)
	...
	Def.ActorFrame {
		...
		Def.Sprite {
			Texture = string.format("icon %s", pad_file)
		}
	}
}

return Def.ActorFrame { preview, card_frame, icon_frame }
